...
class CategoriaCampeonatoList extends TPage
{
    private $form; // form
    ...
    private static $formName = 'formList_CategoriaCampeonato';
    ...
    public function __construct()
    {
        ...
        $row2 = $this->form->addFields([new TLabel('Campeonato:', null, '14px', null)],[$ref_campeonato]);
        ...
        $column_campeonato = new TDataGridColumn('ref_campeonato', 'Campeonato', 'left');

        $column_campeonato->setTransformer( function($ref_campeonato) 
        {
            TTransaction::open('futapp');
            $campeonato = Campeonato::where('id', ' = ', $ref_campeonato)->load();
            TTransaction::close();
        ...
        $this->datagrid->addColumn($column_campeonato);
    
        $btn_onexportcsv = $this->form->addAction('Exportar como CSV', new TAction([$this, 'onExportCsv']), 'fa:file-text-o #000000');

        $btn_onshow = $this->form->addAction('Cadastrar', new TAction(['CategoriaCampeonatoForm', 'onShow']), 'fa:plus #69aa46');
        $action_onShow = new TDataGridAction(array('CategoriaCampeonatoForm', 'onEdit'));
        ...
        $action_onDelete = new TDataGridAction(array('CategoriaCampeonatoList', 'onDelete'));
    ...
    { 
        if(isset($param['delete']) && $param['delete'] == 1)
        {
            try
            {
                ...
                // instantiates object
                $object = new CategoriaCampeonato($key, FALSE); 
    ...
    public function onSearch()
    {
        ...
        if (isset($data->ref_campeonato) AND ( (is_scalar($data->ref_campeonato) AND $data->ref_campeonato !== '') OR (is_array($data->ref_campeonato) AND (!empty($data->ref_campeonato)) )) )
        {
            ...
            $filters[] = new TFilter('ref_campeonato', '=', "{$data->ref_campeonato}");// create the filter 
        }
