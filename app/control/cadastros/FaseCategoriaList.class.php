...
class FaseCategoriaList extends TPage
{
    private $form; // form
    ...
    private static $formName = 'formList_FaseCategoria';
    ...
    public function __construct()
    {
        ...
        $row2 = $this->form->addFields([new TLabel('Categoria:', null, '14px', null)],[$ref_categoria_campeonato]);
        ...
        if ( TSession::getValue('logged') )
        {
          ...
          $btn_onshow = $this->form->addAction('Cadastrar', new TAction(['FaseCategoriaForm', 'onShow']), 'fa:plus #69aa46');
          $action_onShow = new TDataGridAction(array('FaseCategoriaForm', 'onEdit'));
          ...
          $action_onDelete = new TDataGridAction(array('FaseCategoriaList', 'onDelete'));
    ...
    { 
        if(isset($param['delete']) && $param['delete'] == 1)
        {
            try
            {
                ...
                // instantiates object
                $object = new FaseCategoria($key, FALSE); 
    ...
    public function onSearch()
    {
        ...
        {
            $filters[] = new TFilter('ref_categoria_campeonato', 'in', "(select id from categoria_campeonato
                                                                                   where ref_campeonato =  {$data->ref_campeonato})");// create the filter 
        }

        if (isset($data->ref_categoria_campeonato) AND ( (is_scalar($data->ref_categoria_campeonato) AND $data->ref_categoria_campeonato !== '') OR (is_array($data->ref_categoria_campeonato) AND (!empty($data->ref_categoria_campeonato)) )) )
        {
            $filters[] = new TFilter('ref_categoria_campeonato', '=', $data->ref_categoria_campeonato);// create the filter 
        }
        ...
        static function onMudaCampeonato( $param )
    {
        try
        {
            ...
            if (!empty($param['ref_campeonato']))
            {
                ...
                TDBCombo::reloadFromModel('formList_FaseCategoria', 'ref_categoria_campeonato', 'futapp', 'CategoriaCampeonato', 'id', '{nome} ({id})', 'id', $criteria, TRUE);
            }
            else
            {
                TCombo::clearField('formList_FaseCategoria', 'ref_categoria_campeonato');
            }
    ...
    public function fireEvents( $object )
    {
        ...
        if (isset($object->ref_categoria_campeonato)) 
        {
            $obj->ref_categoria_campeonato  = $object->ref_categoria_campeonato;
        }
        TForm::sendData('formList_FaseCategoria', $obj);
    }
