...
class GoleadorForm extends TPage
{
    protected $form;
    private $formFields = [];
    ...
    private static $formName = 'form_Golerador';
    ...
    public function __construct( $param )
    {
        ...
        $row6 = $this->form->addFields([new TLabel('Numero de gols:', null, '14px', null)],[$numero_gols]);
    ...
    public function onSave($param = null) 
    {
        try
        {
            ...
            **/
            ...
            $object = new Goleador(); // create an empty object 
    ...
    public function onEdit( $param )
    {
        try
        {
            if (isset($param['key']))
            {
                ...
                $object = new Goleador($key); // instantiates the Active Record 
    ...
    static function onMudaCampeonato( $param )
    {
        try
        {
            ...
            if (!empty($param['ref_campeonato']))
            {
                ...
                TDBCombo::reloadFromModel('form_Golerador', 'ref_categoria', 'futapp', 'CategoriaCampeonato', 'id', '{nome} ({id})', 'id', $criteria, TRUE);
            }
            else
            {
                TCombo::clearField('form_Golerador', 'ref_categoria');
            }
    ...
    static function onMudaCategoria( $param )
    {
        try
        {
            ...
            if (!empty($param['ref_categoria']))
            {
                ...
                // formname, field, database, model, key, value, ordercolumn = NULL, criteria = NULL, startEmpty = FALSE
                TDBCombo::reloadFromModel('form_Golerador', 'ref_equipe', 'futapp', 'Equipe', 'id', '{nome} ({id})', 'ref_categoria_campeonato', $criteria, TRUE);
            }
            else
            {
                TCombo::clearField('form_Golerador', 'ref_equipe');
            }
    ...
    static function onMudaEquipe( $param )
    {
        try
        {
            ...
            if (!empty($param['ref_equipe']))
            {
                ...
                // formname, field, database, model, key, value, ordercolumn = NULL, criteria = NULL, startEmpty = FALSE
                TDBCombo::reloadFromModel('form_Golerador', 'ref_atleta', 'futapp', 'AtletaEquipe', 'id', '{nome} ({id})', 'ref_equipe', $criteria, TRUE);
            }
            else
            {
                TCombo::clearField('form_Golerador', 'ref_atleta');
            }
    ...
    {
        $obj_Atleta = new AtletaEquipe($object->ref_atleta);

        $Equipe = new Equipe($obj_Atleta->ref_equipe);
        ...
        TForm::sendData('form_Golerador', $obj);
    }
}
